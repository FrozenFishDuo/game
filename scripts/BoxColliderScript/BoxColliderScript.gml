...
function BoxCollider(X, Y, Size) constructor{
		 ...
		 static vectorA = new Vector2(X, Y); // Top Left
		 static vectorB = new Vector2(X + Size, Y); // Top Right
		 static vectorC = new Vector2(X, Y + Size); // Bottom Left
		 static vectorD = new Vector2(X + Size, Y + Size); // Bottom Right
